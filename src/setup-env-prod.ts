...
export async function setUpEnvironmentProd() {
    ...
    console.log(`${data.email} ${data.password} ${data.roles} ${configServiceUrl}`)

    try {
        await axios.post(`${configServiceUrl}/api/users/register`, data, { headers: miniheaders })
        .then(response => {
    ...
    await axios.post(`${configServiceUrl}/api/auth`,
    { email: 'paul@thisco.co', password: import.meta.env.VITE_PAUL_THISCO_ADMIN_PASSWORD }, { headers: miniheaders })
    .then(response => {
        console.log(response.data);
    ...
    await axios.put(`${configServiceUrl}/api/projects`, {
    ...
    // Create branch
    await axios.put(`${configServiceUrl}/api/branches`, { name: branchName, projectName: projectName, gitBranch: branchName }, { headers: headers })
    .then(response => {
        console.log(response.data);
    ...
    // Create environment
    await axios.put(`${configServiceUrl}/api/environments`, { 
        name: 'main', 
